...
@implementation ADHorizontalSwitch
...
- (void)coverPan:(UIPanGestureRecognizer *)sender
{
    ...
    switch (sender.state) {
        case UIGestureRecognizerStateBegan:
            initialX  = self.thumbView.center.x;
            break;
            
        case UIGestureRecognizerStateChanged:
            
            if ( (sender.view.center.x + translatedPoint.x >= self.frame.size.height/2) && (sender.view.center.x + translatedPoint.x <= self.frame.size.width -self.frame.size.height/2)) {
                sender.view.center = CGPointMake(sender.view.center.x + translatedPoint.x, sender.view.center.y);
                
                [sender setTranslation:CGPointMake(0, 0) inView:self];
            }else{
                if(sender.view.center.x + translatedPoint.x > self.frame.size.width -self.frame.size.height/2){
                    sender.view.center = CGPointMake(self.frame.size.width -self.frame.size.height/2, self.frame.size.height/2);
                }else{
                    sender.view.center = CGPointMake(self.frame.size.height/2, self.frame.size.height/2);
        ...
        case UIGestureRecognizerStateEnded: {
            if (sender.view.center.x <= self.frame.size.width/2) {
                ...
                if (initialX > self.frame.size.width/2) {
                    [self close];
                }
...
{
    initialX  = self.thumbView.center.x;
    if(initialX < self.frame.size.width/2){//打开
        [self setOn:YES animated:YES];
        [self open];
    }else{//关闭
        [self setOn:NO animated:YES];
...
{
    if(on){
        self.thumbView.center = CGPointMake(self.frame.size.width -self.frame.size.height/2, self.thumbView.center.y);
        //bgview
        if (self.openBgImage) {
        ...
        self.thumbView.center = CGPointMake(self.frame.size.height/2, self.thumbView.center.y);
